				...
				case 'ecrire':
					if (isset($_POST['nom']))
					{
						...
						if (!empty($_POST['nom']) and !empty($_POST['message']))
						{
							...
							$this->save = 'Votre message � bien �t� enregistr�.';
						}
						if (empty($_POST['nom']))
						{
...
<?php
			...
			else
			{
				...
				if ( $nb_message > $messageParPage )
				{
					...
					if ($pageActuelle!=1)
					{
						...
						echo '<a href="?page='.$page.'&pager='.$pagePrec.'" title="page pr�c�dante"><</a>&nbsp&nbsp;';
					}
					echo '<a href="?page='.$page.'&pager=1" title="premi�re page">1..</a>&nbsp&nbsp;';
					$i=2;
					if ($pageActuelle<=5)
					{
						$i=2;
					...
					if ($pagesTotales!=1)
						echo '<a href="?page='.$page.'&pager='.$pagesTotales.'" title="derni�re page">..'.$pagesTotales.'</a>&nbsp&nbsp;';
		...
		function displayAdmin($page, $action) {
			...
			if ($nb_message!=0)
			{
				...
				echo '<p class="center" ><input type ="submit" name="submit" value="Supprim� les messages s�l�ctionn�s" />';
			...
			{
				echo '<p>Il n\'y a pas encore de message enregistr�.<br/>';
